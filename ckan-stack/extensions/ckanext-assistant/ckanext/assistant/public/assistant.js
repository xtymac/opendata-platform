(function () {
  ...
  style.textContent = `
    ...
    #ckan-assistant-new-btn:hover {
      background: rgba(255, 255, 255, 0.3);
    }

    #ckan-assistant-close {
  ...
  panel.innerHTML = `
    <div id="ckan-assistant-header">
      ...
      <button type="button" id="ckan-assistant-new-btn" aria-label="新对话">+ 新对话</button>
      <button type="button" id="ckan-assistant-close" aria-label="关闭">×</button>
    </div>
  ...
  const newBtn = panel.querySelector('#ckan-assistant-new-btn');
  ...
  function saveConversationToLocalStorage() {
    if (currentConversationId) {
      localStorage.setItem('assistant_conversation_id', currentConversationId);
      localStorage.setItem('assistant_history', JSON.stringify(history));
    }
  }

  function loadConversationFromLocalStorage() {
    const savedId = localStorage.getItem('assistant_conversation_id');
